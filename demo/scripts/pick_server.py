...
class PickActionServer(object):
    ...
    def as_cb(self, goal_map_frame: Pose):
        rospy.loginfo("pick action called for location ", goal_map_frame)
        ...
        rospy.loginfo('Pick action moving to start pose')
        ...
        # Step 2: Position ee in front of requested map location
        ee_pose = self._group.get_current_pose().pose
        pre_pose = PoseStamped()
        pre_pose.pose = deepcopy(goal_map_frame)
        pre_pose.pose.orientation = ee_pose.orientation
        pre_pose.pose.position.x -= 1.0
