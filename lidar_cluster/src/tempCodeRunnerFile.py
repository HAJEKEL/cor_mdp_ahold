...
class ScanProcessor:
    ...
    def scan_callback(self, scan):
        print("scan_callback is running")
        cluster_list, cluster_count = self.db_scan_scan(scan)
    ...
    def db_scan_scan(self, scan):
        db_scan = DBSCAN().fit(scan)  # Use DBSCAN on the scan
        labels = db_scan.labels_
        cluster_count = len(np.unique(labels))  # Count the number of clusters found
        list_of_clusters = [scan[labels == (i-1)] for i in range(cluster_count)]
